...
class Pessoa {
    // Declaração de propriedade (TypeScript)
    nome: string; 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const apresentarBtn = document.getElementById('apresentarBtn') as HTMLButtonElement;
    const saudacaoOutput = document.getElementById('saudacaoOutput') as HTMLDivElement;
    ...
    const displayMessage = (message: string, isSuccess: boolean): void => {
    ...
    nomeInput.addEventListener('keypress', (event: KeyboardEvent) => {
        if (event.key === 'Enter') {
            handleApresentar();
        }
    });
